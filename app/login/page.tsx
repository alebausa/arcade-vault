...
import { authenticateUser, createGuestUser } from "@/data/users";
...
export default function LoginPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    const authedUser = await authenticateUser({ username: user || "PLAYER1", password: pass });
    login(authedUser);
    router.push("/");
  };
  ...
  return (
    <div className="av-auth-wrap fade-in">
      <div className="auth-card">
        ...
        <button className="btn ghost" style={{ width: "100%", marginTop: 10 }} onClick={enterAsGuest}>
          JUGAR COMO INVITADO
        </button>
